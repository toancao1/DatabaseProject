	...
	if (isset($_SESSION["session_flag"])) {
		if ($_SESSION["session_flag"] == "valid") {
			...
			if ($_POST["attribute"] == "OrderID") {
				...
				$sql = 'SELECT OrderID, DateOfOrder FROM Orders WHERE OrderID = ' . $ID;
			} elseif ($_POST["attribute"] == "DateOfOrder") {
				if (isset($_POST["attribute"])) {
					echo 'You chose: ' . $_POST["attribute"] . ' <br>';
					// Escape user inputs for security
					$name = $_POST['attributeValue'];

					echo 'You entered the date: ' . $name . '<br> <br>';
				}
				$sql = 'SELECT OrderID, DateOfOrder FROM Orders WHERE DateOfOrder =  ' . $name;
			}
			...
			while ($row = $result->fetch_assoc()) {
				echo "OrderID: {$row['OrderID']}  <br> " .
					"Date of order: {$row['DateOfOrder']} <br> " .
					"--------------------------------<br>";
			}
